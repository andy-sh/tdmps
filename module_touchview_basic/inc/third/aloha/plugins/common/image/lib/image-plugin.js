...
* 
* $Id: image-plugin.js 78 2012-03-08 08:20:38Z gaox $
*/
...
function AlohaImagePlugin ( aQuery, Plugin, FloatingMenu, i18nCore, i18n ) {
	...
	// Aloha team may want to factorize, it could be useful for other plugins Prototypes
		...
		*/
		initializeButtons: function() {
			...
			if (this.settings.ui.resize) {
				...
				var tabId = this.settings.ui.oneTab ? tabImage : tabImage; // 将修改大小的输入控件放在Image tab下
			...
			var tabId = this.settings.ui.oneTab ? tabImage : tabImage;
			that.__addUIDeleteButton(tabId);
		},
		
		__addUIDeleteButton: function(tabId) {
			var that = this;
			var deleteButton = new Aloha.ui.Button({
				'size' : 'small',
				'tooltip' : i18n.t('button.delete.tooltip'),
				'toggle' : false,
				'iconClass' : 'cnr-delete',
				'onclick' : function (btn, event) {
					that.imageObj.remove();
		 ...
		 */		
 		...
 		_addUIResizeButtons: function (tabId) {
			...
			this.imgResizeHeightField.name = 'imgResizeHeightField';
			...
			this.imgResizeWidthField.name = 'imgResizeWidthField';
...
//					Aloha.Log.error( e, 'Could not disable enableObjectResizing:'+e );
		...
		subscribeEvents: function() {
			...
			Aloha.bind('aloha-selection-changed', function(event, rangeObject, originalEvent) {
				...
				if (Aloha.activeEditable !== null) {
					...
					if (typeof config !== 'undefined' && that.settings.ui.insert) {
		...
		clickImage: function( e ) {
			...
			// 将图片url地址更新到src输入控件
			$('#' + that.imgSrcField.id).val(that.imageObj.attr('src'));
...
//			 this.imgSrcField.getItem(); // (optinal a selected resource item)
			...
			// customize
			this.imageObj.attr('src', $('#' + this.imgSrcField.id).val() ); // (图片更新地址)
		},
		...
		startResize: function () {
			...
			currentImageObj.resizable({
				...
				aspectRatio : that.startAspectRatio,
				handles: that.settings.ui.handles,
